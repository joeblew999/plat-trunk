//! Schema boundary tests — scene domain.
//!
...
use truck_webgpu_gui::commands::{
    AddCubeParams, AddSphereParams, ObjectIdParam, ImportSceneParams,
};
...
fn clear_resets_scene_to_zero_objects() {
    ...
    ctrl.execute("add_sphere", &p(AddSphereParams { radius: 1.0 }));
...
fn delete_removes_exactly_one_object() {
    ...
    let _id_b = object_id_from(&ctrl.execute("add_sphere", &p(AddSphereParams { radius: 1.0 })));
    assert_eq!(ctrl.object_count(), 2);
    ctrl.execute("delete", &p(ObjectIdParam { object_id: id_a.clone() }));
...
fn export_scene_then_import_scene_round_trip_restores_count() {
    ...
    let mut ctrl = HeadlessController::new();
    ctrl.execute("add_cube",   &p(AddCubeParams { size: 1.0 }));
    ctrl.execute("add_sphere", &p(AddSphereParams { radius: 0.5 }));
    ...
    let import_v: serde_json::Value = serde_json::from_str(
        &ctrl2.execute("import_scene", &p(ImportSceneParams { json: scene_json }))).unwrap();
