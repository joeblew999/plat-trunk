 ...
 */
function extractBoundingSphere(entryJson) {
    try {
...
export async function putObject(modelId, objectId, entryJson) {
 ...
 */
export async function bulkPutObjects(modelId, items) {
...
export async function getObject(modelId, objectId) {
    const db = await openObjectDb();
    return new Promise((resolve, reject) => {
...
export async function removeObject(modelId, objectId) {
    const db = await openObjectDb();
    return new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readwrite');
...
export async function listObjects(modelId) {
    const db = await openObjectDb();
    return new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readonly');
        const index = tx.objectStore(STORE_NAME).index('model');
        const req = index.getAllKeys(modelId);
        req.onsuccess = () => resolve(req.result.map(k => k.split('/')[1]));
        req.onerror = () => reject(req.error);
    });
}
 ...
 */
export async function listObjectsWithSpheres(modelId) {
    const db = await openObjectDb();
    return new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readonly');
        const index = tx.objectStore(STORE_NAME).index('model');
        ...
        req.onsuccess = () => {
            resolve((req.result || []).map(r => ({
                objectId: r.objectId,
                boundingSphere: r.boundingSphere ?? null,
            })));
        };
        req.onerror = () => reject(req.error);
    });
...
export async function clearObjects(modelId) {
    const db = await openObjectDb();
    return new Promise((resolve, reject) => {
        const tx = db.transaction(STORE_NAME, 'readwrite');
        const store = tx.objectStore(STORE_NAME);
 ...
 */
export async function evictObject(modelId, objectId) {
 ...
 */
export async function promoteObject(modelId, objectId) {
    const ctrl = window.sceneController;
    if (!ctrl) return null;
