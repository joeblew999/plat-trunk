...
const _schemaReady = fetch('/api/cad/schema')
  ...
  .then(s => {
    _schema = s;
    _jsControlPlane = new Set(Object.keys((s as any).controlPlane || {}));
  })
  ...
  switch (type) {
    ...
    case 'save_cloud': {
      ...
      const { error: saveErr } = await client.PUT('/api/models/{id}', {
        params: { path: { id: mid } },
        body: { name: params.name, scene: sceneJson },
      });
    ...
    case 'delete_model': {
      await client.DELETE('/api/models/{id}', { params: { path: { id: params.id } } });
      (document.querySelector('cad-gallery') as any)?.refresh();
      return { success: true };
    }
...
function cadQuery(type: string, params: any = {}, options: CadOptions = {}) {
...
let _busy = false;
async function cadCommand(type: string, params: any = {}, options: CadOptions = {}) {
