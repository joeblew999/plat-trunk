...
- `web/gui/versions.json` — Version picker data (releases + PR previews), generated by `deploy:versions:json`
...
- PR opened: `task truck:deploy:preview PR_NUMBER={N}` (per-PR preview alias + sticky PR comment)
...
| `https://v0-6-0-truck-cad.gedw99.workers.dev` | Per-version preview (from `deploy:upload`) |
