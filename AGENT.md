...
- `web/gui/versions.json` — Version picker data (releases + PR previews), generated by `cf:versions:json`
...
task cf:promote          # Deploy latest upload to 100% traffic
task cf:rollback         # Roll back to previous version (instant)
task cf:list             # Show all versions + PR previews with URLs
task cf:status           # Show current deployment and traffic split
task cf:upload           # Upload version only (no traffic change)
task truck:deploy:tag          # Create git tag + GitHub release (idempotent)
task cf:canary           # Deploy with gradual traffic split (interactive)
task cf:versions:list    # List recent versions (raw wrangler output)
...
task cf:preview PR_NUMBER=42  # Upload preview → pr-42-truck-cad.gedw99.workers.dev
