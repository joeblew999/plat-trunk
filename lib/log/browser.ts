 ...
 *
 * One-liner setup:
 *   import { setupBrowserLog } from '../../lib/log/browser'
 ...
 * Uses the same LogEntry / Logger types as the worker — entries merge seamlessly in the viewer.
 */
...
export function setupBrowserLog(opts: BrowserLogOptions): BrowserLogSetup {
  ...
  function createLogger(system: string, baseContext?: Record<string, unknown>): Logger {
    ...
    return {
      ...
      child: (ctx) => createLogger(system, { ...base, ...ctx }),
    }
  }
