 ...
 *   import { setupLog, type LogEnv } from '../../lib/log'
 *
...
export interface SetupLogOptions extends LogBufferOptions {
  /** Mount path for debug routes (viewer, tail, API). Default: '/api/debug' */
  debugPath?: string
...
export interface LogUrls {
  ...
  local: { viewer: string; tail: string; api: string }
  /** Production URLs (absolute) — null if no production URL configured */
  production: { viewer: string; tail: string; api: string } | null
...
function buildUrls(debugPath: string, config: ReturnType<typeof buildLogConfig>): LogUrls {
  ...
  const local = { viewer: `${logBase}/viewer`, tail: `${logBase}/tail`, api: logBase }

  const production = config.productionUrl
    ? {
        viewer: `${config.productionUrl}${logBase}/viewer`,
